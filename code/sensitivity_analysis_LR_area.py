...
def main():
    ...
    for var in variation_steps:
        ...
        for P_val, G_val in grade_cenarios:
            matrix, eq = get_nash_equilibrium(R, LR_current, Cs, I, L, alpha, Ca, G_val, P_val)
    ...
    mapa_sensibilidade_LR = pd.concat(all_matrices, ignore_index=True)
    csv_filename = "traceability_sensitivity_LR_areas.csv"
    mapa_sensibilidade_LR.to_csv(csv_filename, index=False)
    print(f"[Info] Full traceability data saved to {csv_filename}")
    
    # Export summarized area data to CSV
    csv_summary = "summary_areas_by_LR_variation.csv"
