...
def main():
    print("--- Sensitivity Analysis of Equilibrium Areas by Judicial Cost (Ca) ---")
    ...
    Ca_base = float(input("Stakeholders base judicial/transaction costs (Ca_base): "))
    ...
    print(f"\nComputing {total_cenarios} analytical iterations for {len(variation_steps)} Ca levels...")
    ...
    # Batch computational evaluation iterating over Ca variations
    for var in variation_steps:
        Ca_current = Ca_base * (1 + var / 100.0)
        
        # Track counts of each equilibrium for the current Ca
        equilibria = []
        matrices_for_var = []
        
        for P_val, G_val in grade_cenarios:
            matrix, eq = get_nash_equilibrium(R, LR, Cs, I, L, alpha, Ca_current, G_val, P_val)
    ...
    mapa_sensibilidade_Ca = pd.concat(all_matrices, ignore_index=True)
    csv_filename = "traceability_sensitivity_Ca_areas.csv"
    mapa_sensibilidade_Ca.to_csv(csv_filename, index=False)
    print(f"[Info] Full traceability data saved to {csv_filename}")
    
    # Export summarized area data to CSV
    csv_summary = "summary_areas_by_Ca_variation.csv"
    ...
    for eq, color in color_map.items():
        ...
        if df_areas[eq].max() > 0:
            plt.plot(df_areas['Ca_Variation_Pct'], df_areas[eq], 
                     color=color, linewidth=2.5, marker='o', markersize=5, label=eq)

    plt.title("Evolution of Equilibrium Areas by Stakeholders Judicial Cost Variation", fontsize=14, fontweight='bold', fontfamily='serif')
    plt.suptitle(f"Base Ca = {Ca_base}", fontsize=11, color='gray', fontfamily='serif', y=0.92)
    plt.xlabel("Variation of Judicial Cost 'Ca' (%)", fontsize=12, fontfamily='serif')
